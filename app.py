...
def get_cheek_landmarks(face_landmarks):
    """頬のランドマークポイントを取得します。"""
    left_cheek = [
        face_landmarks['chin'][1],  # 左頬の上
        face_landmarks['chin'][3],  # 左頬の下
        face_landmarks['top_lip'][1],  # 唇の上（左頬側）
        face_landmarks['left_eye'][4],  # 左目の下
    ]
    right_cheek = [
        face_landmarks['chin'][15],  # 右頬の上
        face_landmarks['chin'][13],  # 右頬の下
        face_landmarks['top_lip'][5],  # 唇の上（右頬側）
        face_landmarks['right_eye'][5],  # 右目の下
    ]
...
def process_image_pair(before_path, after_path):
    ...
    # 頬の明るさも同時に計算
    try:
        before_landmarks_list = face_recognition.face_landmarks(before_image)
        after_landmarks_list = face_recognition.face_landmarks(after_image)
        if not before_landmarks_list:
            raise ValueError("顔が検出されませんでした。")
    except Exception as e:
        print(f"顔認識に失敗しました: {e}")
        return None  # エラーをスキップ

    before_landmarks = before_landmarks_list[0]
    after_landmarks = after_landmarks_list[0]

    left_cheek, right_cheek = get_cheek_landmarks(before_landmarks)
    left_cheek_rgb = extract_cheek_region(before_image, left_cheek)
    right_cheek_rgb = extract_cheek_region(before_image, right_cheek)
    cheek_brightness = calculate_brightness(
        np.vstack([left_cheek_rgb, right_cheek_rgb])
    )

    before_left_cheek, before_right_cheek = get_cheek_landmarks(before_landmarks)
    after_left_cheek, after_right_cheek = get_cheek_landmarks(after_landmarks)
...
def apply_coefficients_multivariate_with_scalers(test_image, models, scaler_X, scaler_Y):
    ...
    try:
        before_landmarks_list = face_recognition.face_landmarks(test_image)
        if not before_landmarks_list:
            raise ValueError("顔が検出されませんでした。")
    except Exception as e:
        print(f"顔認識に失敗しました: {e}")
    ...
    left_cheek, right_cheek = get_cheek_landmarks(before_landmarks)
    left_cheek_rgb = extract_cheek_region(test_image, left_cheek)
    right_cheek_rgb = extract_cheek_region(test_image, right_cheek)
...
# モデルのロード
model_path = 'trained_4factor_model2.pkl'
...
# 更新されたモデルとスケール係数を保存
#updated_model_path = 'updated_4factor_model2.pkl'
...
#output_image_path = 'output_images/adjusted_test1_4factor_2.jpg'  # 出力画像のパス
...
if uploaded_file is not None:
    ...
    #output_image_cv = Image.fromarray(adjusted_image_cv)
    ...
    st.download_button(
        label="調整後の画像をダウンロード",
        #data=cv2.imencode('.jpg', cv2.cvtColor(adjusted_image_cv, cv2.COLOR_BGR2RGB))[1].tobytes(),
